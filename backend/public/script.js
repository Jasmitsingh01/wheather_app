...
function searchWeather() {
  ...
  fetch(apiUrl)
    ...
    .then((data) => {
      ...
      if (condition == "haze") {
        Imagesicon.setAttribute(
          "src",
          "Images/Screenshot 2023-09-22 170505.jpg"
        );
      } else if (condition === "broken clouds") {
        Imagesicon.setAttribute(
          "src",
          "Images/Screenshot 2023-09-22 170426.jpg"
        );
      } else if (condition.includes("clouds")) {
        Imagesicon.setAttribute(
          "src",
          "Images/Screenshot 2023-09-22 170541.jpg"
        );
      } else if (condition == "mist") {
        Imagesicon.setAttribute(
          "src",
          "Images/Screenshot 2023-09-22 170449.jpg"
        );
      } else if (condition.includes("clear ")) {
        Imagesicon.setAttribute(
          "src",
          "Images/Screenshot 2023-09-22 170324.jpg"
        );
      }
    ...
    fetch("/singUp", {
      ...
      .then((jsonResponse) => {
        if (jsonResponse.operation === "Success") {
          ...
          window.location.replace(`/mainapp`);
        } else {
          error.textContent = jsonResponse.message;
        }
      })
...
function Login() {
  ...
  if (
    ...
    fetch("/login", {
      ...
      .then((jsonResponse) => {
        if (jsonResponse.operation === "Sucesss") {
            Error.textContent = " ";
          Error.textContent = jsonResponse.message;
          localStorage.setItem("USER", jsonResponse.Token);
          window.location.replace(`/mainapp`);
    ...
    Error.textContent = "Please Check Deatils";
  }
}
